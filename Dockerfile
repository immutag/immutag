...
#RUN git clone --recursive -j8 https://github.com/immutag/immutag

COPY . /immutag

WORKDIR /immutag/dev

RUN bash -c "chmod +x install_container && yes | ./install_container"

#ENV PATH=/fzf/bin/fzf:$PATH

RUN bash -c "mv /fzf/bin/fzf /root/.local/bin/"

WORKDIR /root/immutag/stage

ENTRYPOINT ["/root/.local/bin/imt"]
